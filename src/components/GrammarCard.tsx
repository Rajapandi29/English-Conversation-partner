    ...
    <div className="bg-slate-900/95 border border-slate-800 rounded-2xl p-4 sm:p-5 shadow-xl space-y-4">
      ...
      <div className="flex items-center justify-between border-b border-slate-800 pb-3">
        ...
        <button
          ...
          className={`px-2.5 py-1 rounded-xl border font-bold text-xs flex items-center gap-1.5 transition-all shrink-0 ${
          ...
          title={
            analysis.overallScore === 100
              ? 'Click to trigger 100% celebratory confetti!'
              : undefined
          }
        >
          {analysis.overallScore === 100 && (
      ...
      <div className="flex items-center gap-1 bg-slate-950/80 p-1 rounded-xl border border-slate-800/80">
        ...
        >
          ...
          <span>
            Fixes ({analysis.corrections.length})
          </span>
        </button>
        ...
        >
          ...
          <span>
            Vocab ({analysis.vocabularyBoosters.length})
          </span>
        </button>
      </div>
          ...
          {analysis.corrections.length === 0 ? (
            ...
            analysis.corrections.map((item, idx) => {
              ...
              return (
                <div
                  key={item.id || idx}
                  className="bg-slate-950/70 border border-slate-800/90 rounded-xl p-3.5 space-y-2.5 hover:border-slate-700 transition-all"
                >
                  {/* Error Type + Actions */}
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      {onSaveToVault && (
                        <button
                          ...
                          className={`text-xs font-semibold px-2.5 py-1 rounded-lg border flex items-center gap-1.5 transition-all cursor-pointer ${
                            isSaved
                              ? 'bg-emerald-500/20 text-emerald-300 border-emerald-500/40 hover:bg-emerald-500/30 shadow-sm shadow-emerald-950/50'
                              : 'bg-indigo-500/15 hover:bg-indigo-500/25 text-indigo-300 border-indigo-500/30 hover:border-indigo-400/50 hover:scale-[1.02] active:scale-[0.98]'
                          }`}
                          title={
                            isSaved
                              ? 'Rule saved in Vault. Click to toggle.'
                              : 'Save this grammar rule to Vault for later review'
                          }
                        >
                          {isSaved ? (
                            <>
                      ...
                      {onPracticeSentence && (
                        <button
                          type="button"
                          onClick={() =>
                            onPracticeSentence(item.corrected)
                          }
                          className="text-[11px] text-slate-300 hover:text-white px-2 py-1 rounded-lg bg-slate-800/70 hover:bg-slate-700/80 border border-slate-700/60 flex items-center gap-1 font-medium transition-all"
                          title="Practice saying this corrected phrase"
                        >
                          <span>Practice</span>
                  ...
                  <div className="space-y-1 text-xs">
                    <div className="flex items-start gap-2 text-rose-400/90">
                      <span className="text-[10px] font-bold uppercase text-slate-500 pt-0.5 shrink-0">
                        Incorrect:
                      </span>

                      <span className="line-through decoration-rose-500 font-mono bg-rose-500/10 px-1.5 py-0.5 rounded break-words">
                        {item.original}
                      </span>
                    </div>
                    ...
                    <div className="flex items-start gap-2 text-emerald-400">
                      ...
                      <span className="font-semibold font-mono bg-emerald-500/10 px-1.5 py-0.5 rounded break-words">
                        {item.corrected}
                      </span>
                    </div>
                  </div>
        ...
        <div className="space-y-3">
          ...
          <div className="bg-slate-950/70 border border-slate-800 rounded-xl p-3.5 space-y-2">
            <div className="flex items-center gap-1.5 text-xs font-semibold text-indigo-400">
              ...
              <span>
                Professional & Polished (Workplace/Interview):
              </span>
            </div>
      ...
      {activeTab === 'vocab' && (
        <div className="space-y-2.5">
          {analysis.vocabularyBoosters.length === 0 ? (
            <div className="p-4 rounded-xl bg-slate-950/70 border border-slate-800 text-center">
              ...
              <p className="text-xs font-semibold text-slate-300">
                No vocabulary boosters available.
              </p>
            </div>
          ) : (
